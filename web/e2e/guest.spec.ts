...
test('learn and portfolio navigation work before playing', async ({ page }) => {
  await page.goto('/'); await page.getByRole('button', { name: 'How to play', exact: true }).click();
  ...
  await expect(page.getByRole('heading', { name: 'Every portfolio starts with a first move.' })).toBeVisible();
});
test('setup can be dismissed and restores focus', async ({ page }) => {
  await page.goto('/'); const trigger = page.getByRole('button', { name: 'Let’s play', exact: true });
  await trigger.click(); await page.keyboard.press('Escape'); await expect(page.getByRole('dialog')).toHaveCount(0); await expect(trigger).toBeFocused();
});
