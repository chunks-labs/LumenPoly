...
export function netWorth(match: Match, id: PlayerId): number {
  return (
    match.players[id].balance +
    properties(match, id).reduce((total, tile) => total + (tile.price ?? 0), 0)
  );
}
export function rentFor(match: Match, tileId: number): number {
  ...
  const group = lumenPolyBoard.filter(
    (item) => item.type === 'property' && item.color === tile.color,
  );
  const complete =
    owner && group.length > 0 && group.every((item) => match.owners[item.id] === owner);
