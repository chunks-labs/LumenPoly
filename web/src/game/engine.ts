...
export function transition(current: Match, action: GameAction, random = Math.random): Match {
  ...
  if ((action.type === 'buy' || action.type === 'end') && current.phase !== 'resolve') return current;
  if ('dice' in action && !validDice(action.dice)) return current;
  const match = structuredClone(current);
  if (action.type === 'buy') { purchase(match, 'you'); return match; }
  if (action.type === 'end') { match.pendingProperty = null; match.phase = 'bot'; return match; }
